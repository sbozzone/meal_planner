...
export default function ShoppingPage() {
  ...
  const { weekStart, weekLabel } = useWeekNavigation();
  const {
    items,
    loading,
    addItem,
    ...
    <>
      ...
      <div className="px-4 py-3 max-w-3xl mx-auto space-y-4">
        ...
        <button
          ...
          disabled={generating}
          className="w-full flex items-center justify-center gap-2 py-2.5 border border-accent/30 text-accent rounded-lg text-sm font-medium hover:bg-accent-light/30 active:scale-[0.98] transition-all disabled:opacity-50 min-h-touch"
        >
          <Sparkles className="w-4 h-4" />
          {generating ? "Generating..." : `Generate from ${weekLabel}`}
        </button>
                    ...
                    return (
                      <div key={cat}>
                        <button
                          ...
                          className="flex items-center gap-2 mb-1.5 px-1 py-1 min-h-touch"
                        >
                          {isCollapsed ? (
              ...
              <div>
                ...
                <div className="space-y-1">
                  {checked.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-3 px-4 py-2.5 bg-card/50 border border-border-light/50 rounded-card"
                    >
                      <button
                      ...
                      <button
                        ...
                        className="p-2 rounded text-text-muted hover:text-red transition-all"
                      >
